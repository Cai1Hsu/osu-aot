...
namespace osu.Framework.Configuration
{
    ...
    internal class InputConfigManager : ConfigManager
    {
        ...
        private void bindToHandlersBindables()
        {
            ...
            foreach (var handler in InputHandlers)
            {
                foreach (var property in handler.GetType().GetProperties())
                {
                    ...
                    // get the OnValueChanged method of the InputHandler instance
                    var onValueChangedMethod = handler.GetType().GetMethod(nameof(OnValueChanged), BindingFlags.NonPublic | BindingFlags.Instance);

                    // get the subscribe method of the InputConfigManager instance
                    var subscribeMethod = typeof(InputConfigManager).GetMethod(nameof(subscribe), BindingFlags.NonPublic | BindingFlags.Instance);
                    Debug.Assert(subscribeMethod != null);
                    Debug.Assert(onValueChangedMethod != null);

                    // create a delegate for the OnValueChanged method
                    var onValueChangedDelegate = Delegate.CreateDelegate(typeof(Action<Bindable<object>, object, object>), handler, onValueChangedMethod);

                    // create a delegate for the subscribe method
                    var subscribeDelegate = Delegate.CreateDelegate(typeof(Action<Bindable<object>, Action<object, object>>), this, subscribeMethod.MakeGenericMethod(encapsulatedType));

                    // call the subscribe method with the bindable and the OnValueChanged delegate
                    subscribeDelegate.DynamicInvoke(property.GetValue(handler), onValueChangedDelegate);
                }
            }
        ...
        private void OnValueChanged<T>(Bindable<T> bindable, T oldValue, T newValue)
        {
            QueueBackgroundSave();
        }
